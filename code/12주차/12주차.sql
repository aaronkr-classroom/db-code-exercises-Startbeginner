...
-- 3 : 아이템 가격은 0원이상이여야 한다
ALTER TABLE items ADD CONSTRAINT chk_item_price
CHECK (price>=0);

UPDATE items SET price=-10 WHERE item_id=1001;

-- 4 : 아이템 등급을 정해진 값만 가능하다 (S, A, B, C, D, E, F)
ALTER TABLE items ADD CONSTRAINT chk_item_grade
CHECK (grade in ('S', 'A', 'B', 'C', 'D', 'E', 'F'));

-- 5 : 닉네임은 중복되면 안 된다
ALTER TABLE users ADD CONSTRAINT uq_user_nickname
UNIQUE (nickname);
